...
public class NetworkManager : MonoBehaviourPun
{
    ...
    public void SendAlterTheFutureCardsChanged(int actorNumber, int[] cards) => photonView.RPC(nameof(AlterTheFutureCardsChanged), RpcTarget.All, actorNumber, cards);
    ...
    [PunRPC] private void AlterTheFutureCardsChanged(int actorNumber, int[] cards) => OnAlterTheFutureCardsChanged?.Invoke(actorNumber, cards);
}
